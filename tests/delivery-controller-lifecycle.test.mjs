...
test("pickup materializes durable authority then dispatch resolves only its immutable work item", async (t) => {
  const root = await project(t); await manifest(root); const value = adapter(root, bus()); const record = await value.pickup("AIDEV-77"); await assert.rejects(value.dispatchWorkItem(record.handle), /dispatch_not_bound/); const item = { id: "AIDEV-77", source: "sources/local", branch: "feature/77", baseRef: "main", verificationContract: "test", instructions: ["do work"] }; assert.deepEqual(await value.bindDispatch(record.handle, "dispatch-77", "correlation-77"), item); assert.deepEqual(await value.bindDispatch(record.handle, "dispatch-77", "correlation-77"), item); await assert.rejects(value.bindDispatch(record.handle, "other-77", "correlation-77"), /dispatch_identity_conflict/); assert.deepEqual(await value.dispatchWorkItem(record.handle), item);
  assert.equal(typeof value.dispatch, "undefined"); const raw = await readFile(new URL("../extensions/delivery-controller/src/index.ts", import.meta.url), "utf8"); assert.match(raw, /lifecycleHandle/); assert.match(raw, /bindDispatch/); assert.equal(/item:\s*workItem/.test(raw), false);
});
