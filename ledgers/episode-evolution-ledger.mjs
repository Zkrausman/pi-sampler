...
export class EpisodeEvolutionLedger {
  ...
  constructor({ root, limits = {}, trustedAuthorityIds = [], verifyAttestation, faultInjector } = {}) {
    ...
    this.episodes = new Map(); this.eventIds = new Map(); this.identityOwners = new Map(); this.storageBytes = 0; this.reservedStorageBytes = 0; this.pending = 0; this.queues = new Map(); this.closed = false; this.closing = false; this.usable = true;
  }
  ...
  async backup({ maxBytes = this.limits.maxExportBytes } = {}) { return this.#enqueue("admission", async () => { await this.#reconcile(); const files = []; let total = 0; const add = async (relative, source) => { const b = await readFile(source); total += b.byteLength; if (total > maxBytes || total > this.limits.maxExportBytes) throw new LedgerLimitError("maxExportBytes", total, Math.min(maxBytes, this.limits.maxExportBytes)); files.push({ path: relative, size: b.byteLength, digest: hash(b), body: b }); }; await add("ledger-manifest.json", this.#path("manifest.json")); for (const top of ["commits", "commits-v2", "artifacts", "quarantine"]) { const walk = async (p, rel) => { for (const e of await this.#sortedEntries(p)) { const q = join(p, e.name); if (e.isDirectory() && !e.isSymbolicLink()) await walk(q, `${rel}/${e.name}`); else if (e.isFile() && !e.isSymbolicLink()) await add(`${rel}/${e.name}`, q); else throw new LedgerError("backup_unsafe_path", "backup source contains unsafe entry"); } }; await walk(this.#path(top), top); } const anchor = digest(files.map(({ path, size, digest: d }) => ({ path, size, digest: d }))), dir = this.#path("backups", anchor), meta = stable({ format: "episode-evolution-ledger-backup", version: 1, anchor, files: files.map(({ path, size, digest: d }) => ({ path, size, digest: d })) }), publications = [...files.map((f) => ({ target: join(dir, ...f.path.split("/")), contents: f.body })), { target: join(dir, "backup-manifest.json"), contents: meta }];
