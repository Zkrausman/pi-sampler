...
func winIdentityFromHandle(handle syscall.Handle, info syscall.ByHandleFileInformation) externalIdentity {
	...
	return externalIdentity{
		Device: device, File: low, FileHigh: high, Links: uint64(info.NumberOfLinks),
		Size: int64(uint64(info.FileSizeHigh)<<32 | uint64(info.FileSizeLow)), Mode: info.FileAttributes, Modified: int64(info.LastWriteTime.HighDateTime)<<32 | int64(uint32(info.LastWriteTime.LowDateTime)),
