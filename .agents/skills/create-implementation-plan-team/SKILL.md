...
This skill turns you into a pure Orchestrator. Your role is not to write the plan yourself, but to coordinate a dedicated 3-agent team using the registered subagent tool.
...
### 2. Spawn The Researcher (Inherited model, low thinking)
*   **Action**: Use the subagent tool to spawn the officially registered `codebase-researcher` subagent. It does not pin a model; pi-subagents applies configured defaults or overrides, otherwise inheriting the parent session model.
*   **Prompt**: *"Read the ticket details for [TICKET-ID] and map the affected boundaries."*

### 3. Spawn The Lead Architect (Inherited model, high thinking)
*   **Action**: Once the Researcher returns their map, use the subagent tool to spawn the officially registered `lead-architect` subagent. It does not pin a model; pi-subagents applies configured defaults or overrides, otherwise inheriting the parent session model.
*   **Prompt**: *"Using the Researcher's map and ticket requirements, draft the initial Implementation Plan for [TICKET-ID]."*

### 4. Spawn The Red-Teamer (Inherited model, high thinking)
*   **Action**: Take the Architect's draft and pass it through the subagent tool to the officially registered `adversarial-red-teamer` subagent. It does not pin a model; pi-subagents applies configured defaults or overrides, otherwise inheriting the parent session model.
...
*Note for Platform Engines: The role definitions intentionally omit provider-specific model IDs. Runtime configuration or the parent session selects the model while each role retains its explicit thinking level.*
