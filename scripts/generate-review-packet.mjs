...
function immutableEndpoint(blob, filePath, endpoint) {
  if (!blob) return null;
  if (blob.byteLength > LIMITS.immutableEndpoint) fail(`${filePath} cannot embed complete ${endpoint} immutable blob: it exceeds ${LIMITS.immutableEndpoint} bytes; split or reduce the change so reviewers can verify the complete committed endpoint`);
  return blob;
}
