...
const LIMITS = Object.freeze({ argument: 4096, branch: 256, sha: 64, body: 24 * 1024, markerJson: 4096 });
const TICKET_BRANCH = /^zkrausman\/aidev-[1-9][0-9]*-[a-z0-9]+(?:-[a-z0-9]+)*$/;
const SHA = /^(?:[0-9a-f]{40}|[0-9a-f]{64})$/;
const MARKER_TAG = "<!-- pi-sampler-adversarial-review-attestation:v1";
const MARKER = /<!-- pi-sampler-adversarial-review-attestation:v1 ([^\r\n]{1,4096}) -->/g;
const FORMAT = "pi-sampler.adversarial-review-attestation";
const ALLOWED_REVIEWER_ROLES = new Set(["independent-human-reviewer", "independent-fresh-context-reviewer"]);
const EXPECTED_KEYS = ["base", "format", "head", "outcome", "packetSha256", "reviewerRole", "version"];
...
function validateSchema(attestation, { base, head, packetSha256 }) {
  ...
  if (attestation.format !== FORMAT || attestation.version !== 1) fail("review attestation format or version is unsupported");
  ...
  if (!ALLOWED_REVIEWER_ROLES.has(attestation.reviewerRole)) fail("review attestation reviewer role is not an allowed independent reviewer role");
...
/** Validate a PR body without executing or persisting any of its contents. */
...
async function main() {
  try {
    ...
    // Never echo the PR body or parsed marker: it may contain private material.
