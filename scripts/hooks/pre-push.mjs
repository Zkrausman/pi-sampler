...
function prRecord(branch) {
  let text;
  try {
    text = safeExec("gh", ["pr", "view", branch, "--json", "baseRefOid,headRefName,body"], { maxBuffer: MAX_GH_OUTPUT_BYTES });
  } catch (error) {
  ...
  if (keys.join(",") !== "baseRefOid,body,headRefName") throw new Error("GitHub response has an unexpected shape");
...
function validateMarker({ branch, head, base, body }) {
...
try {
  ...
  for (const candidate of candidates) {
    ...
    try {
      validateMarker({ branch: candidate.branch, head: candidate.head, base: pr.baseRefOid, body: pr.body ?? "" });
