...
This complete two-file pair is an uncommitted, pre-Slice-1 packet-safety planning amendment only. The pair at trusted base `56e494dcc11b5df701c421ad68d240290d649c6a` is authoritative history, but its one-file controller boundary cannot satisfy trusted packet-v3. Independent approval of this amendment remains mandatory; planning grants no implementation, commit, push, PR/tracker mutation, marker, merge, cleanup, or quarantine authority. `do not merge` remains sticky; only `Merge PR #N` authorizes that merge.

| Fact | Value |
|---|---|
| Repository/profile | `Zkrausman/pi-sampler`; `profiles/pi-sampler.json` |
| Trusted amendment base | `56e494dcc11b5df701c421ad68d240290d649c6a` |
...
Mandatory order: (1) validate and independently approve this exact two-file amendment against `56e494dcc11b5df701c421ad68d240290d649c6a`; (2) only separately authorized publication may merge the amendment and capture full `PLAN_AMENDMENT_SHA`; (3) that same-ticket merge output, not the candidate plan text or old implementation branch, becomes the only exact Slice 1 base and stales prior implementation head/review evidence; (4) re-create the inert Slice 1 as an exact descendant with the 13-path allowlist in §5 and both controllers packet-safe; (5) merge output captures `SLICE1_SHA`/`SLICE1_OUTPUT_SHA256` and makes this pair stale; (6) rewrite/revalidate the complete pair against `SLICE1_SHA` and renew independent approval; (7) Slice 2 transition from exact `SLICE1_SHA`; (8) activated validation only after Slice 2 merge; (9) refresh/review PR #172. Same-ticket outputs are explicit bootstrap/transition gates, never manifest self-dependencies.
...
Authority split: authenticated `scripts/trusted-delivery-evidence-controller.mjs` remains the sole executable CLI entry, mode dispatcher, authority orchestrator, trusted-path selector, and trusted planning-validator invoker. New `scripts/trusted-delivery-evidence-controller-core.mjs` is a non-executable internal module containing only cohesive canonical-byte, fixed Git/process, safe path/root identity, inventory, and exact-blob primitives selected by the entry module. It has no shebang or `main`, does not inspect `process.argv`, has no mode/activation/profile/policy/schema/evidence/lifecycle path set, and cannot select a tool, contract, candidate, verifier, or authority. Imports are one-way: the entry has no static core import and the core never imports the entry. In transition/validate, the entry's retained minimal fixed-Git/hash bootstrap authenticates the exact-base `100644` blobs and trusted-worktree bytes of both controller files before a dynamic `import()` of the core and before any candidate-root read. Support may dynamically import the candidate core only after exact 13-path scope/status inventory is frozen; that path remains explicitly functional-only and cannot create trusted-path, transition, validation, evidence, or lifecycle authority. The entry re-exports the existing tested JS surface `main,parseTrustedDeliveryArgs,locateFixedGit,readTrustedBlob,runTrustedPlanValidator,buildNormalizedFacts,canonicalJSONString,sha256Bytes`; direct core exports are internal mechanical helpers only. Go `acceptance_v2.go` owns matrix/evidence/policy evaluation from bound facts. JS never decides satisfaction; Go never selects live repository contracts. Go exports remain `AcceptanceMatrixV2,AcceptanceMatrixV2Row,AcceptanceEvidenceV2,AcceptanceArtifactV2,NormalizedFactsV1,AcceptanceResultV1,ValidateAcceptanceV2`; platform `OpenExternalEvidenceRoot,ReadVerifiedArtifact`; low-level `ParseImplementationPlanManifestV2Compatibility` always has `delivery_admitted=false`.

### 2.1 Mode-specific authority

**Support (Slice 1 only):** exact base is `PLAN_AMENDMENT_SHA`, the reviewed amendment merge output; `56e494dcc11b5df701c421ad68d240290d649c6a` and failed head `65f1e3b6b8e520077b7adbb01036c500f63ca0e0` are history, not implementation authority. The base contains neither v2 controller file. Candidate runs exactly:
...
Canonical report keys `format,version,status,authority,base_sha,head_sha,paths,test_report_sha256,repository_inventory_sha256`; constants `pi-sampler.delivery-v2-support-report`,1,`functional-only`,false. `paths` is exactly these six ordered values: `scripts/trusted-delivery-evidence-controller.mjs`, `scripts/trusted-delivery-evidence-controller-core.mjs`, `tests/delivery-acceptance.test.mjs`, `tests/delivery-acceptance-v2.test.mjs`, `scripts/run-governance-tests.mjs`, `package.json`; tests assert count 6 and exact order. It cannot invoke transition/validate or grant authority; amendment-base review-policy/workspace/packet/protected-CI plus independent review are sole authority.
...
Slice 1 base is exactly full `PLAN_AMENDMENT_SHA`, captured only from the separately authorized merge of this approved two-file amendment. Before mutation, prove `56e494dcc11b5df701c421ad68d240290d649c6a` is its parent/ancestor as publication policy requires, both planned controller paths are absent at that base, the checkout is clean, and the 13-path allowlist is §5 Slice1. Never continue the failed implementation branch in place. Head `65f1e3b6b8e520077b7adbb01036c500f63ca0e0`, approval `0728dea8366832e9ea99275919c7acf2f90f3b90c59f638ded89a114db3ef2b6`, and their packet failure are historical only. The locally verified packet-safe `acceptance_v2.go` and Node test refactors may be reapplied only as reviewable source bytes in the new descendant and must pass all current inventory, behavior, and packet checks; prior reports cannot authorize or attest the new head.

Exact amendment-base preflight is `(cd "$BASE_ROOT" && npm test)` under Node major 24 and `(cd "$BASE_ROOT/governance" && go test -race ./...)` under Go `1.25.0`; both must exit 0 and retain exact stdout/stderr SHA-256. PowerShell uses the same literal argv and cwd semantics (`Set-Location`/`git -C`, `node.exe`, `go.exe`), never a substituted shell command. Implement inert support with declaration/map absent/profile digest unchanged. First split the controller at the boundary in §2, then update the 13-path candidate allowlist, six-path support report, transition predecessor/map count 12, candidate scope checks, and tests before any transition/validate exercise. Run §2 support commands, retain the functional-only report, and complete amendment-base independent review/protected CI. Candidate support is not authority.

Before review, measure Git's exact `--unified=3` hunks from `PLAN_AMENDMENT_SHA` to the proposed Slice1 head. Every new-file hunk must be <=65,536 bytes and every physical source line <=4,096 bytes; each controller hunk must be <=61,440 bytes to retain review margin. Both files must remain readable source modules: no minification, generated code, dense long-line packing, test/behavior removal, packet-limit change, or split that moves policy/authority into core. Generate and validate complete packet v3 for all 13 paths; `incomplete:false`, no omitted/truncated/material arrays, and exact base/head binding are mandatory.

After merge, capture full `SLICE1_SHA`, prove `PLAN_AMENDMENT_SHA` ancestor and support report head; output drift stales pair. Refresh all base/ticket/plan/contract/runtime/schema/profile/validator/controller-pair/JIT digests and renew approval. Slice 2 allowlist is §5 Slice2. Run §2 transition: authenticate all 12 predecessor paths before candidate reads, prove trusted declaration/map absent, validate candidate declaration/map and candidate path allowlist, complete tests, no self-validation. Receipt has `will-activate-after-merge`. Post-merge activated base alone may validate.
...
**Slice 1 New (9):** `governance/pkg/deliveryevidence/{acceptance_v2.go,external_root_posix.go,external_root_windows.go}`; `governance/docs/delivery-evidence/acceptance-matrix-v2.schema.json`; `scripts/{trusted-delivery-evidence-controller.mjs,trusted-delivery-evidence-controller-core.mjs}`; `tests/delivery-acceptance-v2.test.mjs`; `tests/fixtures/delivery-acceptance-v2/{aidev-187-implementation-plan.md,aidev-187-acceptance-manifest-v2.json}`.

**Slice 1 Modified (4):** `governance/cmd/delivery-evidence-validator/main.go`; `governance/pkg/deliveryevidence/validator_test.go`; `tests/delivery-acceptance.test.mjs`; `package.json` (script only, no dependency/lock change). Exact Slice 1 candidate scope is these 13 paths: no rename/copy/delete, no other path, both new controller files mode `100644`, and every New/Modified path belongs to Slice 1 exactly once.
...
- [ ] A191-T04: Select activation, implementation-plan contract, validator, profile, policy, schema, controller entry, controller core, and Go runtime only from verified regular blobs at the exact trusted base.
  Route: Slice1 support is functional only. Tests assert no static core import, one-way dependency, entry-only CLI/authority constants, stable entry re-exports, exact 13-path scope, exact six-path support report, and <=61,440-byte entry/core Git hunks. Refreshed Slice2 uses §2 transition: 12 predecessor blobs valid; declaration/map absent trusted but valid candidate blobs; hostile candidate entry/core/schema/profile ignored. Pre-activation validate=`activation_absent`; post-merge authenticates map/declaration and both controller records before dynamic import/candidate reads. Wrong path/mode/object/size/digest/absence fails with a candidate-read sentinel unchanged; report -> Slice2 review.
...
- [ ] A191-T12: Preserve packet-v3, receipt-v1, marker-v3, revocation, fixed pre-push receipt, trusted-base CI, privacy, DCO, and sticky user-only merge authority unchanged.
  Route: exact blob-map command §6; exact `git diff --unified=3 PLAN_AMENDMENT_SHA Slice1Head -- <each-new-path>` hunk/line checks; complete packet generation and `node scripts/validate-review-packet.mjs --packet <external-packet> --base PLAN_AMENDMENT_SHA --head Slice1Head`; `node --test tests/final-review-receipt.test.mjs tests/adversarial-review-attestation.test.mjs`; `node scripts/validate-dco.mjs`; `npm test`. Assert 13 changed paths, both controller blobs/hunks, full map/absence, golden bytes, same-head revocation, fixed receipt, base workflow, privacy and merge profile; candidate authority substitution, minification, generated bytes, overlong lines, omission, or packet-limit weakening fails. Report required by both reviews.

## 9. Staleness/JIT/completion

Current pair is ready only as the two-file corrective amendment against exact base `56e494dcc11b5df701c421ad68d240290d649c6a`; it does not authorize Slice 1. Plan, ticket, amendment base, controller boundary/path set, any affected contract/requirement/activation, approval expiry, amendment merge output, or Slice1 output drift requires complete refresh and renewed approval as applicable. JIT rechecks exact current base, ticket revision, final plan digest, manifest contract `9c31dc84e57dfd691b3e2a45046b7ab9b2876e51a08e223556bb7e4350824a0e`, validator `3bea29df796b108ba284ffed25094855253599bc7fe4d8c5c2a920b972ea62fc`, profile `96e4b00bc78b16b5e544ee48f369137c74a2ef040c7b226b5c88d542d2e6a6c9`, profile schema `7c83a3d308d5b7e193a7333316c55cdbfbbc9b3e181b6fadd4f67fba63fae954`, and `scoped-review-packet/v3` packet generator `11ebb005703f69a4431e4a28fdc050409a442e340cc09902c82a348272bff2b2`. After amendment merge, record `PLAN_AMENDMENT_SHA`; before Slice2 replace all bindings with exact Slice1 outputs in a newly validated/approved pair. No external hard/predecessor dependency; AIDEV-187 downstream only; unresolved human decisions none. Completion requires plan <61,440 bytes, manifest digest binding, validator `ok:true`/12/zero diagnostics, packet-v3 generation and validation for exactly the two amended planning paths, exact two-file worktree status, and fresh independent approval. Historical failed/blocked heads and reports remain history only. No implementation or lifecycle action follows automatically.
