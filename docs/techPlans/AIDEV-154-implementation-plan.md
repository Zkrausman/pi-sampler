...
*   `[NEW]` `tests/pi-answer-package.test.mjs`: Verify the fork’s fixed audited-source fingerprints, command contract, package metadata, packed artifact, attribution, and clean consumer installation behavior.
...
1.  **Phase 1: Establish the approved distributable-package boundary**
    *   Step 1.1: Confirm that `@zkrausman/pi-answer` is available in the intended public npm registry and record the registry/ownership decision in the package metadata and release documentation; stop rather than silently choosing a different name or private registry.
...
3.  **Phase 3: Document and test the consumer contract**
    ...
    *   Step 3.3: Add Node-test-runner coverage that checks the extension manifest’s public distribution contract, required packed files, upstream attribution/MIT text, command registration, and fixed SHA-256 fingerprints for the audited `pi-answer@0.1.9` source files. Preserve a versioned source-provenance manifest or fixture (upstream repository, directory, release version, immutable artifact/source digest, retrieval date) in the package so a reviewer can reproduce the baseline evidence without CI reading the ignored local copy.
    *   Step 3.4: Add an isolated consumer-install scenario that packs the workspace package, installs that tarball with npm in a temporary project, and verifies its manifest, declared dependency graph, and Pi TypeScript entry are present without any `workspace:` protocol or local pi-sampler path. Do not require Node to directly import the TypeScript Pi entry without Pi’s loader; retain the repository Pi smoke validator for command registration.
    *   Step 3.5: Add a Pi-runtime unit/integration test with a stubbed model registry/UI that proves `/answer` remains TUI-only, chooses only an authenticated configured model or the active model, and sends only the selected last complete assistant message to `modelRegistry.complete`; assert that package installation/packing causes no extraction request, subprocess, telemetry, or network request.
    *   Step 3.6: Create a pending Changeset for the initial `0.1.0` release and regenerate deterministic compliance artifacts only after the final manifest is settled.
...
*   **Validation Scenarios**:
    ...
    *   [ ] The repository Pi smoke test registers `/answer`; its copied source matches fixed SHA-256 fingerprints from the audited upstream baseline, while the test itself does not require ignored local source material.
