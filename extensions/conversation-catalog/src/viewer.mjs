  ...
  const server = createServer(async (request, response) => {
    ...
    if (noteMatch) {
      ...
      try {
        ...
        if (request.method === "DELETE" && noteMatch[3]) { const all = (await readHindsightNotes(session.projectRoot, reference))?.notes || []; if (!all.some((note) => note.noteId === noteMatch[3] && note.eventReference === event.noteReference)) return genericError(response); await deleteHindsightNote(session.projectRoot, reference, noteMatch[3]); return response.end(JSON.stringify({ notes: await currentNotes() })); }
      } catch { return genericError(response, 400); }
      return genericError(response);
    }
