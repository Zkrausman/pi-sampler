...
import { normalizeHindsightNarrativeMap, renderHindsightNarrativeMapHtml } from "./hindsight-narrative-map.mjs";
...
export function generateCitedHindsightDocumentHtml(document) {
  ...
  const narrativeMapEnabled = document?.narrativeMapEnabled === true;
  const narrativeMapExcluded = document?.narrativeMapExcluded === true;
  if (document?.narrativeMap !== undefined && !narrativeMapEnabled) {
    throw new Error("Narrative maps require explicit opt-in.");
  }
  if (narrativeMapExcluded && document?.narrativeMap !== undefined) {
    throw new Error("An excluded conversation can render only a content-free narrative-map placeholder.");
  }
  const narrativeMap = narrativeMapEnabled && !narrativeMapExcluded
    ? normalizeHindsightNarrativeMap(document?.narrativeMap, evidenceByReference)
    : undefined;
  ...
  const narrativeMapHtml = !narrativeMapEnabled ? "" : narrativeMapExcluded
    ? '<p class="empty" role="status">Narrative map unavailable: the selected conversation was excluded during redaction review.</p>'
    : renderHindsightNarrativeMapHtml(narrativeMap, citationLinks, new Map(evidence.map((item, index) => [item.reference, index])));
  ...
  const topActions = recommendations.map((recommendation, index) => ({ recommendation, index }))
    .sort((left, right) => priorityOrder[left.recommendation.priority] - priorityOrder[right.recommendation.priority] || left.index - right.index)
    .slice(0, 3).map(({ recommendation }) => recommendation);
  const summaryHtml = `<p class="model-generated">Model-generated reading summary; it is not a user-confirmed conclusion.</p><p>This report contains ${normalizedClaims.length} cited claim${normalizedClaims.length === 1 ? "" : "s"} (${normalizedClaims.filter((claim) => claim.classification === "direct evidence").length} direct-evidence, ${normalizedClaims.filter((claim) => claim.classification === "inference").length} inference) and ${recommendations.length} model-proposed recommendation${recommendations.length === 1 ? "" : "s"}.</p>`;
  const topActionsHtml = topActions.length === 0 ? '<p class="empty" role="status">No structured recommendations were supplied.</p>' : `<div class="action-grid">${topActions.map((recommendation, index) => `<article class="action-card"><p class="classification">${escapeHtml(recommendation.priority)} priority · model suggestion</p><h3>${index + 1}. ${escapeHtml(recommendation.recommendation)}</h3><dl><dt>Impact</dt><dd>${escapeHtml(recommendation.expectedImpact)}</dd><dt>Suggested owner</dt><dd>${escapeHtml(recommendation.suggestedOwner)}</dd><dt>Done criteria</dt><dd>${list(recommendation.acceptanceCriteria, "No acceptance criteria supplied")}</dd></dl><p class="citations">Evidence: ${citationLinks(recommendation.references)}</p></article>`).join("")}</div>`;
  const timelineSteps = storySteps.length > 0 ? storySteps.slice(0, 7) : normalizedClaims.slice(0, 7).map((claim, index) => ({ title: `Claim ${index + 1}`, body: claim.statement, classification: claim.classification, references: claim.references }));
  const timelineHtml = timelineSteps.length === 0 ? '<p class="empty" role="status">No cited story steps or claims were supplied for a narrative timeline.</p>' : `<p class="model-generated">${storySteps.length > 0 ? "Model-generated narrative guide" : "Safe fallback assembled from cited claims"}; classifications distinguish evidence from inference.</p><ol class="compact-timeline">${timelineSteps.map((step) => `<li><article><p class="classification">${step.classification === "inference" ? "Inference" : "Direct evidence"}</p><h3>${escapeHtml(step.title)}</h3><p>${escapeHtml(step.body)}</p><p class="citations">${citationLinks(step.references)}</p></article></li>`).join("")}</ol>`;
  const insightList = (items, label, empty) => items.length === 0 ? `<p class="empty">${empty}</p>` : `<ul class="insight-list">${items.map((claim) => `<li><p class="classification">${label}</p><p>${escapeHtml(claim.statement)}</p><p class="citations">${citationLinks(claim.references)}</p></li>`).join("")}</ul>`;
  const appendixHtml = `<section id="evidence-appendix" aria-labelledby="evidence-appendix-heading"><h2 id="evidence-appendix-heading">Evidence appendix</h2><p class="empty">Expanded material remains in this standalone report; sections are closed by default for reader-first review.</p><details><summary>Claims (${normalizedClaims.length})</summary>${claimHtml}</details><details><summary>Claim-support validation</summary>${claimSupportHtml}</details><details><summary>All recommendations and local dispositions (${recommendations.length})</summary>${recommendationHtml}</details><details><summary>User-observed outcomes and feedback</summary>${outcomeHistoryPlaceholder}${feedbackPlaceholder}${priorOutcomeHtml}</details><details><summary>User-authored notes</summary>${userAuthoredNotesHtml || '<p class="empty">No user-authored notes were supplied.</p>'}</details><details><summary>Embedded redacted evidence (${evidence.length})</summary>${evidenceHtml}</details></section>`;
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; script-src 'unsafe-inline'; base-uri 'none'; form-action 'none'"><title>Pi hindsight document</title><style>:root{color-scheme:dark;font-family:system-ui,sans-serif;background:#1a1b26;color:#c0caf5}html{scroll-behavior:smooth}body{margin:0;line-height:1.45}.report-shell{display:grid;grid-template-columns:15rem minmax(0,65rem);gap:2rem;margin:0 auto;max-width:86rem;padding:2rem}.report-toc{align-self:start;position:sticky;top:1rem;background:#24283b;border:1px solid #414868;border-radius:.5rem;padding:1rem}.report-toc h2{font-size:1rem;margin-top:0}.report-toc ul{list-style:none;margin:0;padding:0}.report-toc li+li{margin-top:.5rem}.report-toc a{color:#c0caf5}.report-main section{scroll-margin-top:1rem}.reader-section{margin:0 0 2rem}.claim,.evidence,.story-step article,.narrative-map-node,.narrative-map-relationships,.claim-support-list li,.disposition-panel,.disposition-export,.action-card,.compact-timeline article,.insight-list li{background:#24283b;border:1px solid #414868;border-radius:.5rem;margin:1rem 0;padding:1rem}.metrics,.action-grid{display:grid;gap:1rem;grid-template-columns:repeat(auto-fit,minmax(10rem,1fr))}.metric{background:#24283b;border:1px solid #414868;border-radius:.5rem;padding:1rem}.metric strong{display:block;font-size:1.5rem}.metric span,.model-generated{color:#a9b1d6}.action-card h3,.compact-timeline h3{margin:.2rem 0}.action-card dl{display:grid;grid-template-columns:max-content 1fr;gap:.35rem .75rem}.action-card dt{font-weight:700}.action-card dd{margin:0}.compact-timeline{padding-left:1.5rem}.insight-list{padding-left:1.25rem}.insight-list li{border-left:.5rem solid #2c78c4}.claim-inference{border-left:.5rem solid #a66b12}.claim-direct-evidence{border-left:.5rem solid #2c78c4}.classification,.provenance,.story-provenance,.model-generated{font-size:.85rem;font-weight:700;text-transform:capitalize}.story-provenance{color:#a9b1d6}.story-reading-order{padding-left:1.5rem}.story-step{margin:1rem 0;padding-left:.25rem}.story-inference article{border-left:.5rem solid #a66b12}.story-direct-evidence article{border-left:.5rem solid #2c78c4}.narrative-map-groups{display:grid;gap:1rem;grid-template-columns:repeat(auto-fit,minmax(16rem,1fr))}.narrative-map-group{border:1px solid #414868;border-radius:.5rem;padding:1rem}.narrative-map-group h3{margin-top:0}.narrative-map-group ol{padding-left:1.5rem}.narrative-map-node{margin:.75rem 0}.narrative-map-node h4{margin-top:0}.narrative-map-inference{border-left:.5rem solid #a66b12}.narrative-map-direct-relationship{border-left:.5rem solid #2c78c4}.narrative-map-edges{padding-left:1.5rem}.narrative-map-edges li{margin:.75rem 0}.narrative-map-arrow{font-weight:700}.story-filter{align-items:center;display:flex;flex-wrap:wrap;gap:.75rem}.story-filter p{margin:.25rem 0}.citations a{font-weight:700}.evidence-chip{background:#2f354d;border:1px solid #414868;border-radius:1rem;display:inline-block;margin:.15rem .2rem .15rem 0;padding:.12rem .5rem;text-decoration:none}.context{white-space:pre-wrap;overflow-wrap:anywhere}.context-panel{border-top:1px dashed #414868;margin-top:.9rem;padding-top:.2rem;scroll-margin-top:1rem}.context-panel h4,.evidence h4{margin-bottom:.25rem}.evidence-unavailable{border-left:.5rem solid #a66b12}.context-fallback,.empty{color:#a9b1d6}.claim-support-list{list-style-position:inside;padding:0}.table-scroll{overflow-x:auto;margin:1rem 0}.table-scroll:focus,.disposition-form :focus-visible,button:focus-visible,a:focus-visible,summary:focus-visible{outline:2px solid #7aa2f7;outline-offset:2px}table{border-collapse:collapse;min-width:72rem;width:100%;background:#24283b}caption{text-align:left;font-weight:700;padding:.5rem 0}th,td{border:1px solid #414868;padding:.7rem;text-align:left;vertical-align:top;overflow-wrap:anywhere}thead th{background:#2f354d}tbody th{min-width:14rem}td ul{margin:.1rem 0;padding-left:1.2rem}.disposition-form{border-top:1px solid #414868;margin-top:1rem;padding-top:1rem}.disposition-form:first-of-type{border-top:0;margin-top:0}.disposition-form fieldset{border:0;margin:0;padding:0}.disposition-options{display:flex;flex-wrap:wrap;gap:1rem}.disposition-options label{font-weight:700}.disposition-form textarea{box-sizing:border-box;display:block;min-height:5rem;max-width:48rem;width:100%}.disposition-actions{align-items:center;display:flex;flex-wrap:wrap;gap:.75rem;margin-top:.75rem}.disposition-status{font-weight:700}.model-provenance{color:#a9b1d6}details{background:#24283b;border:1px solid #414868;border-radius:.5rem;margin:1rem 0;padding:.75rem}summary{cursor:pointer;font-weight:700}@media(max-width:52rem){.report-shell{display:block;padding:1rem}.report-toc{position:static;margin-bottom:1.5rem}.report-toc ul{display:flex;flex-wrap:wrap;gap:.5rem 1rem}.report-toc li+li{margin-top:0}}</style></head><body><div class="report-shell"><nav class="report-toc" aria-label="Report contents"><h2>On this page</h2><ul><li><a href="#summary">Summary</a></li><li><a href="#context">Context</a></li><li><a href="#top-actions">Top actions</a></li><li><a href="#what-happened">What happened</a></li><li><a href="#what-worked">What worked</a></li><li><a href="#lessons-risks">Lessons &amp; risks</a></li><li><a href="#visualizations">Visualizations</a></li><li><a href="#evidence-appendix">Evidence appendix</a></li></ul></nav><main class="report-main"><header><h1>${escapeHtml(bounded(document?.title, "Pi hindsight document", 160))}</h1></header><section id="summary" class="reader-section"><h2>Summary</h2>${summaryHtml}</section><section id="context" class="reader-section"><h2>Context at a glance</h2><p class="empty">Counts describe only the redacted report bundle; no raw session identity is displayed.</p><div class="metrics"><div class="metric"><strong>${evidence.length}</strong><span>redacted evidence excerpts</span></div><div class="metric"><strong>${normalizedClaims.length}</strong><span>cited claims</span></div><div class="metric"><strong>${recommendations.length}</strong><span>model-proposed recommendations</span></div><div class="metric"><strong>${storySteps.length}</strong><span>story steps supplied</span></div></div></section><section id="top-actions" class="reader-section"><h2>Top actions</h2><p class="model-generated">Deterministically selected by priority, then original recommendation order. Suggested owners are model-proposed, not user-confirmed.</p>${topActionsHtml}</section><section id="what-happened" class="reader-section"><h2>What happened</h2>${timelineHtml}${storySteps.length > 0 ? `<details><summary>Evidence-first story reading order</summary>${storyHtml}</details>` : ""}</section><section id="what-worked" class="reader-section"><h2>What worked</h2><p class="model-generated">Evidence-backed strengths drawn from existing direct-evidence claims.</p>${insightList(normalizedClaims.filter((claim) => claim.classification === "direct evidence").slice(0, 3), "Direct evidence", "No direct-evidence claims were supplied.")}</section><section id="lessons-risks" class="reader-section"><h2>Lessons &amp; risks</h2><p class="model-generated">Model inferences are interpretations, not user-confirmed findings.</p>${insightList(normalizedClaims.filter((claim) => claim.classification === "inference").slice(0, 3), "Inference", "No inference claims were supplied.")}</section><section id="visualizations" class="reader-section"><h2>Visualizations</h2>${narrativeMapEnabled ? `<section><h3>Cited narrative map</h3>${narrativeMapHtml}</section>` : '<p class="empty">No narrative map was requested for this report.</p>'}</section>${appendixHtml}</main></div>${storyFilterScript}${recommendations.length === 0 ? "" : dispositionScript}</body></html>`;
}
