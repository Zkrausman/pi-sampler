...
function ticketCloseoutDescriptor(value) {
  if (value === undefined) return undefined;
  if (!value || typeof value !== "object" || Array.isArray(value) || Object.keys(value).some((key) => !["version", "ticket", "pickedUpAt", "closedAt", "durationMs", "coverage", "completedSegments", "totalSegments", "totals", "gaps", "mergedEvidenceCount", "closedEvidenceCount"].includes(key)) || value.version !== 1 || typeof value.ticket !== "string" || !/^[A-Z][A-Z0-9]{1,9}-[1-9][0-9]{0,8}$/.test(value.ticket) || typeof value.pickedUpAt !== "string" || typeof value.closedAt !== "string" || Number.isNaN(Date.parse(value.pickedUpAt)) || Number.isNaN(Date.parse(value.closedAt)) || !Number.isSafeInteger(value.durationMs) || value.durationMs < 0 || value.durationMs !== Date.parse(value.closedAt) - Date.parse(value.pickedUpAt) || !["complete", "partial"].includes(value.coverage) || !Number.isSafeInteger(value.completedSegments) || !Number.isSafeInteger(value.totalSegments) || value.completedSegments < 0 || value.totalSegments < 1 || value.completedSegments > value.totalSegments || !value.totals || !Number.isSafeInteger(value.totals.total) || !Number.isSafeInteger(value.totals.parentDelta) || !Number.isSafeInteger(value.totals.subagentTotal) || !Number.isSafeInteger(value.totals.subagentRuns) || value.totals.total !== value.totals.parentDelta + value.totals.subagentTotal || !Array.isArray(value.gaps) || value.gaps.some((gap) => !["interrupted", "missing-receiver", "settle-failed", "abandoned"].includes(gap)) || !Number.isSafeInteger(value.mergedEvidenceCount) || !Number.isSafeInteger(value.closedEvidenceCount) || value.mergedEvidenceCount < 1 || value.closedEvidenceCount < 1 || (value.coverage === "complete") !== (value.gaps.length === 0)) throw new Error("Ticket closeout descriptor is malformed.");
