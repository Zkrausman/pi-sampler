...
export { DEFAULT_SYNTHESIS_CHUNK_BYTES, planCanonicalSynthesisChunks } from "./chunk-plan.mjs";
...
function synthesisPromptParts(sources, { hindsightNotes = undefined } = {}) {
  ...
  const evidenceText = JSON.stringify(promptEvidence); const notesText = JSON.stringify(userAuthoredNotes);
  const prompt = `Create a rigorous hindsight analysis from ONLY the selected conversation's redacted source bundle below. Every material claim and recommendation must cite exact included evidence references. Do not invent facts or use content outside this bundle.\n\nDo NOT write HTML or use a file-writing tool. Call hindsight_document_write exactly once with a short title, structured claims, optional structured storySteps, and structured recommendations. Claims and storySteps must be classified direct evidence or inference and cite included evidence. Recommendations must include actionType ("fix" or "harden"), recommendation, priority (critical, high, medium, or low), expectedImpact, suggestedOwner, dependencies, acceptanceCriteria, status "proposed", source "model-suggestion", and evidenceReferences. For every surfaced direct-evidence strength, include at least one Harden proposal sharing one of its cited evidence references. For every surfaced inference lesson or risk, include at least one Fix proposal sharing one of its cited evidence references. The safe contract escapes model text and creates all local citation links and evidence context.${subagentInstruction}\n\nREDACTED SOURCE BUNDLE:\n${evidenceText}\n\nEXCLUDED SELECTION REFERENCES (do not cite):\n${JSON.stringify(excluded)}

${notesInstruction}

USER-AUTHORED HINDSIGHT NOTES (context only; never evidence/citations):
${notesText}`;
  const evidenceBytes = Buffer.byteLength(evidenceText, "utf8"); const noteContextBytes = Buffer.byteLength(notesText, "utf8"); const totalBytes = Buffer.byteLength(prompt, "utf8"); return { prompt, evidenceBytes, noteContextBytes, instructionsAndOverheadBytes: totalBytes - evidenceBytes - noteContextBytes, totalBytes };
}
export function buildSynthesisPrompt(sources, options = {}) { return synthesisPromptParts(sources, options).prompt; }
export function measureSynthesisPrompt(sources, options = {}) { const { evidenceBytes, noteContextBytes, instructionsAndOverheadBytes, totalBytes } = synthesisPromptParts(sources, options); return { evidenceBytes, noteContextBytes, instructionsAndOverheadBytes, totalBytes }; }
export function preflightSynthesisPrompt(sources, options = {}, contextUsage) { const { prompt, evidenceBytes, noteContextBytes, instructionsAndOverheadBytes, totalBytes } = synthesisPromptParts(sources, options); if (totalBytes > MAX_SYNTHESIS_PROMPT_BYTES) { const noteGuidance = noteContextBytes > evidenceBytes ? " Review, redact, or remove hindsight notes, then retry." : " Redact or select a shorter conversation, then retry."; throw new Error(`Hindsight prompt is ${totalBytes.toLocaleString()} bytes (selected redacted evidence: ${evidenceBytes.toLocaleString()} bytes; note context: ${noteContextBytes.toLocaleString()} bytes; instructions/overhead: ${instructionsAndOverheadBytes.toLocaleString()} bytes); submissions are limited to ${MAX_SYNTHESIS_PROMPT_BYTES.toLocaleString()} bytes. This direct single-prompt preflight does not perform chunk synthesis; /hindsight-document uses the bounded chunk workflow for eligible included conversations.${noteGuidance}`); } const window = contextUsage?.contextWindow; const used = contextUsage?.tokens; if (!Number.isFinite(window) || window <= 0 || !Number.isFinite(used) || used < 0) throw new Error("Unable to determine the active model's context capacity. Run /compact (or start a fresh session), send a short message, then retry hindsight generation."); const available = Math.floor(window - used - SYNTHESIS_RESPONSE_RESERVE_TOKENS); if (available < totalBytes) throw new Error(`Hindsight needs up to ${totalBytes.toLocaleString()} conservative context tokens, but only ${Math.max(0, available).toLocaleString()} are available after reserving room for the report. Run /compact or start a fresh session, then retry.`); return prompt; }
