...
export function buildHindsightDocument(sources, modelOutput = undefined) {
  const selected = selectedSources(sources); const evidence = sourceEvidence(selected); const allowed = new Set(evidence.filter((item) => item.availability !== "excluded").map((item) => item.reference)); const contract = subagentEvidenceContract(evidence); const model = modelOutput === undefined ? { claims: defaultClaims(selected), storySteps: [], recommendations: [], subagentEfficiency: undefined } : validateModelOutput(modelOutput, allowed, contract);
  return generateCitedHindsightDocumentHtml({ title: model.title || "Hindsight source bundle — selected conversation", claims: [...model.claims, ...fallbackClaims(selected)], storySteps: model.storySteps, recommendations: model.recommendations, subagentEfficiency: model.subagentEfficiency, evidence });
}
export function buildSynthesisPrompt(sources) {
  const selected = selectedSources(sources); const evidence = sourceEvidence(selected); const included = evidence.filter((item) => item.availability !== "excluded"); const promptEvidence = included.map(({ delegationPair: _delegationPair, ...item }) => item); const excluded = evidence.filter((item) => item.availability === "excluded").map((item) => item.reference); const contract = subagentEvidenceContract(included); const qualified = [...contract.timing]; const deliveryPairs = contract.deliveryPairs.map((pair) => ({ resultEvidenceReferences: [...pair.results], chronologicalFollowUpEvidenceReferences: [...pair.followUps] }));
