...
/hindsight-document [--validate-claim-support] [--narrative-map] [--prior-outcomes <report.outcomes.json>] [output-path]
/hindsight-notes
...
Pass `--narrative-map` to opt into an optional cited narrative map. Without this flag, the safe writer rejects a `narrativeMap` field. With it, the model may provide only a bounded, text-only structured hint: the one supported `chronological` layout, 1–12 simple-ID groups, 1–30 nodes, and 0–50 directed edges. Every substantive node and edge must be explicitly classified as `direct relationship` or `inference`, cite 1–3 unique included redacted event references, and use only known group/node IDs. Unknown, duplicate, excluded, note/outcome/feedback, or malformed citations are rejected; model HTML, SVG, JavaScript, URLs-as-directives, unsupported layouts, and extra directives are rejected rather than rendered.

The fixed standalone local renderer owns all HTML/CSS and chronological presentation. It escapes all accepted text, labels inference relationships, orders groups/nodes by earliest cited event while stating that order is not causation, provides citation deep links to embedded redacted evidence, and retains a semantic no-JavaScript reading structure. It has no remote assets or network capability and uses a restrictive CSP. If the selected conversation was excluded during redaction review, the map section is only a content-free unavailable placeholder; no model map is accepted.
