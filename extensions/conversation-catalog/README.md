...
`/hindsight-notes` is an opt-in, interactive **current-session-only** workflow to add, view, edit, or delete short user-authored notes. Notes are stored separately from Pi session logs at `.pi/hindsight-notes/<opaque-session-reference>.json`; the reference is a truncated SHA-256 digest of the actual Pi session ID (never its display name), so it is stable and collision-resistant without writing a raw ID/name. Persistence APIs derive this path from the project root and session reference internally. On Linux they use pinned directory descriptors, `O_NOFOLLOW`, descriptor-relative temporary writes/renames, and non-recursive lock removal, so parent or leaf symlink swaps cannot redirect an operation outside the trusted store. On Windows, where Node has no equivalent safe descriptor-relative file API, notes use a validated `SystemRoot`-derived `System32\\reg.exe` with fixed direct argument arrays (never a shell) under `HKCU\\Software\\Zkrausman\\PiConversationCatalog\\HindsightNotes\\v1`. SHA-256 digests of the canonical trusted project root and session form the key scope; a bounded index value and hashed per-note atomic values support all 100 legal notes without whole-key reads. Raw project paths, session IDs, and note IDs never become Registry paths or value names. A per-session Windows named-pipe mutex serializes index/note CRUD; it has no mutable filesystem path and is released by the OS if its process exits. Linux uses a local cross-process lock. Both backends reject malformed bounded records and preserve unrelated notes during concurrent updates.
