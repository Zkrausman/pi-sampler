...
`/hindsight-notes` is an opt-in, interactive **current-session-only** workflow to add, view, edit, or delete short user-authored notes. Notes are stored separately from Pi session logs at `.pi/hindsight-notes/<opaque-session-reference>.json`; the reference is a truncated SHA-256 digest of the actual Pi session ID (never its display name), so it is stable and collision-resistant without writing a raw ID/name. Persistence APIs derive this path from the project root and session reference internally. On Linux they use pinned directory descriptors, `O_NOFOLLOW`, descriptor-relative temporary writes/renames, and non-recursive lock removal, so parent or leaf symlink swaps cannot redirect an operation outside the trusted store. On Windows, where Node has no equivalent safe descriptor-relative file API, notes use only fixed direct `C:\\Windows\\System32\\reg.exe` argument arrays (never a shell) under `HKCU\\Software\\Zkrausman\\PiConversationCatalog\\HindsightNotes\\v1`. SHA-256 digests of the canonical trusted project root and session form the key scope; hashed note IDs form per-note atomic values. Raw project paths, session IDs, and note IDs never become Registry paths or value names. The versioned store accepts only bounded, user-authored/user-confirmed note records and rejects recognizable credentials and raw session identifiers. Linux uses a local cross-process lock; Windows independently updates bounded per-note Registry values so concurrent CRUD cannot replace unrelated notes.
